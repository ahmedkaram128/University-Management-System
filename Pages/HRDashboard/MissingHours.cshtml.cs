...
namespace University_HR_ManagementSystem.Pages.HRDashboard
{
    public class MissingHoursModel : PageModel
    {
        ...
        public MissingHoursModel(IConfiguration config)
        {
            _config = config;
        }
        ...
        public async Task<IActionResult> OnPostAddDeductionAsync()
        {
            ...
            try
            {
                var procCmd = new SqlCommand("Deduction_hours", conn)
                {
            ...
            var fetchCmd = new SqlCommand(@"
                SELECT TOP 1 *
                FROM Deduction
                WHERE emp_ID = @id AND type = 'missing_hours' AND date = CAST(GETDATE() AS DATE)
            ...
            {
                ModelState.AddModelError("EmployeeID", "No deduction was created — employee has no missing hours.");
                MonthlyDeductions = await GetCurrentMonthDeductionsAsync();
                return Page();
            }
