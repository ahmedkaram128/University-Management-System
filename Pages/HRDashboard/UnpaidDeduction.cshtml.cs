...
{
    public class UnpaidDeductionModel : PageModel
    {
        private readonly IConfiguration _config;

        public UnpaidDeductionModel(IConfiguration config)
        {
            _config = config;
        }
        ...
        public async Task<IActionResult> OnPostAddDeductionAsync()
        {
            ...
            try
            {
                var procCmd = new SqlCommand("Deduction_unpaid", conn)
                {
            ...
            var fetchCmd = new SqlCommand(@"
                            SELECT TOP 1 *
                            FROM Deduction
                            WHERE emp_ID = @id AND type = 'unpaid' AND date = CAST(GETDATE() AS DATE)
            ...
            {
                ModelState.AddModelError("EmployeeID", "No deduction was created — employee has no unpaid leaves.");
                MonthlyDeductions = await GetCurrentMonthDeductionsAsync();
                return Page();
            }
            ...
            // Refresh table for current month
            MonthlyDeductions = await GetCurrentMonthDeductionsAsync();
            return RedirectToPage();
        }

        // ------------------------------------------------------------------------
        // HELPER: Load current month unpaid deductions
        // ------------------------------------------------------------------------
