...
{
    public class ManageCompensationModel : PageModel
    {
        private readonly IConfiguration _config;

        public ManageCompensationModel(IConfiguration config)
        {
            _config = config;
        }
        ...
        public List<SelectListItem> CompensationLeaves { get; set; } = new();
        ...
        public async Task OnGet()
        {
            ...
            await LoadCompensationRequests(hrId);
        }
        ...
        public async Task<IActionResult> OnPost()
        {
            ...
            try
            {
                ...
                using var cmd = new SqlCommand("HR_approval_comp", conn);
            ...
            await LoadCompensationRequests(hrId); // reload list
            return Page();
        }

        private async Task LoadCompensationRequests(string hrId)
        {
            CompensationLeaves.Clear();
            ...
            SELECT L.request_ID,
                   CONCAT('Request ID: ', L.request_ID,' - Start Date: ', L.start_date,
                    ...
                    ' - Employee ID: ', C.Emp_ID, ' - Replacement ID: ', c.replacement_emp, ' - Original work day: ', c.date_of_original_workday) AS DisplayText
            FROM Employee_Approve_Leave EAL
            JOIN [Leave] L ON L.request_ID = EAL.leave_ID
            JOIN Compensation_Leave C ON C.request_ID = L.request_ID
            ...
            {
                CompensationLeaves.Add(new SelectListItem
                {
