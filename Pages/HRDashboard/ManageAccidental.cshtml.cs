...
{
    public class ManageAccidentalModel : PageModel
    {
        private readonly IConfiguration _config;

        public ManageAccidentalModel(IConfiguration config)
        {
            _config = config;
        }
        ...
        public List<SelectListItem> AccidentalLeaves { get; set; } = new();
        ...
        public async Task OnGet()
        {
            ...
            await LoadAccidentalRequests(hrId);
        }
        ...
        public async Task<IActionResult> OnPost()
        {
            ...
            await LoadAccidentalRequests(hrId); // reload list
            return Page();
        }

        private async Task LoadAccidentalRequests(string hrId)
        {
            AccidentalLeaves.Clear();
            ...
            SELECT L.request_ID,
                   CONCAT('Request ID: ', L.request_ID,' - Start Date: ', L.start_date,
                    ...
                    ' - Employee ID: ', A.Emp_ID) AS DisplayText
            FROM Employee_Approve_Leave EAL
            JOIN [Leave] L ON L.request_ID = EAL.leave_ID
            JOIN Accidental_Leave A ON A.request_ID = L.request_ID
            ...
            {
                AccidentalLeaves.Add(new SelectListItem
                {
